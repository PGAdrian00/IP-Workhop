            ...
            let serverListener = server.listen (process.env.PORT || 6969, function () {
                ...
                for (let network in networks) {
                    for (let networkAddress of (networks as any)[network])
                    {
                        ...
                        {
                            n = n + 1;
                            console.log ("Server running at http://"+networkAddress.address+":"+(serverListener.address() as AddressInfo).port);
                        }
                    }
                }
                if (n === 0)
                {
                    console.log ("Server running at http://127.0.0.1:"+(serverListener.address() as AddressInfo).port);
                }
            });
